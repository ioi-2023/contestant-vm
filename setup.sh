...
# - webcam
# - WiFi + Bluetooth card
# These are hardware components that are not needed for the constest, therefore we disable their drivers.

# YOU MIGHT NEED TO UPDATE THIS IF YOU WILL BE USING A DIFFERENT LAPTOP, OR IF THE OS CHANGES. Therefore, here is an explanation how this part of the configuration was created. Otherwise, it would be quite magical why certain kernel modules have been disabled.

# It is important to note that it is good practice to use ethernet during a competition since it is more stable than WiFi. We will also use ethernet on this year's IOI.

# While we were trying out the notebook, we have noticed that the WiFi + Bluetooth did not work due to problems with drivers on Linux. Which is probably a feature in this case. I am not sure whether it is possible that an update could remove this "feature", therefore I am disabling its drivers altogether. (Check https://community.intel.com/t5/Wireless/AX101-Ubuntu-22-04-or-22-10-driver/td-p/1468063)
# Command `sudo dmesg` showed that there have been attempts on starting up WiFi and Bluetooth.

# Some additional fun-facts: in the IOI 2023 laptop, there is an Intel Wi-Fi 6 AX101 card.
...
# To disable bluetooth, I disabled the bluetooth kernel module and everything else depending on it recursively. Probably disabling only one module would be sufficient, I am not sure, I had problems if only the `bluetooth` module was disabled.
...
# From dmesg I got a hint of the name for the WiFi card's kernel module is `iwlwifi`. Then I have went it with the exact same method as in bluetooth (described above).
# Disable wifi
